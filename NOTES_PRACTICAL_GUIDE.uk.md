...
Цей режим бере текст із буфера і намагається зчитати контекст активної апки.
...
- тому в UI Gyazo бачиш плейсхолдер `Input name of App`.

Якщо хочеш реальну апку в metadata:
- зроби текст у буфері;
- запусти `./notes_capture_from_clipboard.sh` без аргументів, коли потрібна апка активна.

Формат `desc` для notes:
- перший рядок у коментарі Gyazo лишається змістовним (заголовок/текст);
- тег апки додається в останній рядок із тегами (наприклад: `#notes #Safari`).
